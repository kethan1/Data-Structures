...
'''
Why Numpy?
Numpy arrays are more efficient (they take up less memory and are faster) for larger 
datasets with the same type. Numpy arrays also provide you with lots to ways to
...
# Converts the string to a number, error if it cannot convert to number
array_to_manipulate = np.array([1, 2, 3, "9"], dtype="i4") # Array filled with 32 bit signed integers
...
# This may be preferred to np.ones or np.zeros, because when creating an empty array with an
# some number of values, and it doesn't matter what those values are, it is faster
print(np.empty(3))

print("Using np.arange")
# np.arange acts exactly like range, except that it returns an array with the values
print(np.arange(4))
print(np.arange(0, 30, 2))

print("Using np.linspace")
# Returns a numpy array with the number of values specified with num evenly gapped, including
# start and stop
...
# When reshaping an array, make sure that the reshaped array has the same amount of elements
...
# Using a < 5 or something similar compares each element in the array and returns
# True or False depending on whether each element satifies that condition
print(arr6[arr6 < 5])
print(arr6[arr6 == 5])
...
# You can also do there operations with one integer, the integer will be applied
